...
<?php
	...
	global $MaxD, $MaxF, $MaxH, $MaxM, $MaxT, $MaxV, $MaxY;
	$link_id = db_connect();
	...
	Function PlotProfileMonitor($plt) {  
		...
		if ($plt == "trh") {
			...
			$row = mysqli_fetch_row($rs); 
			$prev[0] = $row[0];
			ImageLine($imageT,0,255-($prev[0]/$MaxT)*255,0,255-($prev[0]/$MaxT)*255,$red);
			$prev[1] = $row[1];
			...
			for ($i = 1; $i< $cntr; $i++) { 
				...
				mysqli_data_seek($rs, $i);
				$row = mysqli_fetch_row($rs); 
				ImageLine($imageT,$j,255-($prev[0]/$MaxT)*255,$k,255-($row[0]/$MaxT)*255,$red);
		...
		if ($plt == "dbf") {
			...
			mysqli_data_seek($rs, 0);
			$row = mysqli_fetch_row($rs); 
			$MaxD = 130;  //  minus 125 to plus 130 = range 255
			$MaxF = 22050;  // 44100 / 2
			...
			ImageLine($imageD,0,130,720,130,$black);     
			...
			$row = mysqli_fetch_row($rs); 
			$prev[0] = $row[0];
			ImageLine($imageD,0,130-($prev[0]/$MaxD)*130,0,130-($prev[0]/$MaxD)*130,$red);
			$prev[1] = $row[1];
			...
			for ($i = 1; $i< $cntr; $i++) { 
				...
				$row = mysqli_fetch_row($rs); 
				$dat[0] = $row[0];
				ImageLine($imageD,$j,130-($prev[0]/$MaxD)*130,$k,130-($row[0]/$MaxD)*130,$red);
				$dat[1] = $row[10];
    ...
    <h3>Oil and Gas Monitor Plots</h3>
		<ul>		
			...
			<li>Temperature and Relative Humidity</li>
				...
				Temperature (Red)<?php echo " 0 to $MaxT deg F" ?> -- Relative Humidity (Blue)<?php echo " 0 to $MaxH %" ?> </br>
				</br>
			<li>Sound: Decibels and Frequency</li>			
				<?php PlotProfileMonitor("dbf") ?> 
				<IMG SRC="imageD.png"></br>
				Decibels (Red) -125 to +130 db -- Frequency (Blue) 0 to 22050 hz </br>
				</br>
			<li>Particles: 1.0, 2.5, 10.0</li>
